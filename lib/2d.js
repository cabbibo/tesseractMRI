function Controller2D( params ){
  ...
  // A Corner defines which of the equations
  // that make it up, by referencing their indexes
  this.corners = [
...
Controller2D.prototype = {
  ...
  loopThroughIntersections: function(){
    ...
    if( this.intersections.length == 2 ){
      ...
      var d = this.intersections[0].distanceTo( this.intersections[1] );
  ...
  getCornerPosition: function( corner ){


    // This basically says:
    // Look at the which one of the equations makes up this corner
    // corner[0] will always refer to equations of x
    // corner[1] will always refer to equations of y
    // and [2] of this equations gives us what it is 
    // equal to.
    var x = this.equations[ corner[0] ][2];
    var y = this.equations[ corner[1] ][2];

    return this.convertToThree( [ x , y ] );



  },

  // Checks to see which corner has been hit
  checkCornerHit: function( corner , type){

    console.log( corner );
    for( var i = 0; i < this.corners.length; i++ ){

      console.log( this.corners[i] );
      var intersection = _.intersection( corner , this.corners[i] );

      // Sees if the intersection, is the corner
      if( M.arraysEqual( this.corners[i] , intersection ) ){

        var cornerPosition = this.getCornerPosition( this.corners[i] );

        this.renderer.hitCorner( cornerPosition , type );
 ...
 *  by Math2D
 *
 *  By the time any positions are passed into the renderer,
 *  they should be in a THREE.Vector3 form.
 *
 */
...
Renderer2D.prototype = {
  ...
  hitCorner: function( position , type ){

    console.log( 'check' );

    this.controller.particleEmitter.position = position;

    this.controller.particleEmitter.birth();
